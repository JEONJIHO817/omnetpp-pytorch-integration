...
void SimpleLoRaApp::initialize(int stage)
{
    ...
    model = torch::jit::load("../../model/diabetes_clf.pt");
...
//    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *>(msg);
    ...
    //=============================================================
    std::ifstream infile("../../model/input_data.txt");
    if (!infile.is_open()) {
        std::cout << "[EDGE MODEL] Cannot open input_data.txt" << endl;
        delete msg;
        return;
    }
    std::string line;
    ...
    while (std::getline(infile, line)) {
        ...
        std::vector<float> mean = {3.742671009771987, 120.85504885993485, 69.41530944625407, 20.39902280130293, 81.43811074918567, 31.98338763333299, 0.4691677532410583, 32.9071661237785};
        std::vector<float> scale = {3.310564970320035, 32.008958928832904, 18.497517048413847, 15.421400979340468, 116.14014299402147, 7.734319028151803, 0.3365723332189259, 11.494065064569593};

        for (int i = 0; i < 8; i++) {
            values[i] = (values[i] - mean[i]) / scale[i];
        }

        torch::Tensor input = torch::tensor(values).unsqueeze(0);
        torch::Tensor output = model.forward({input}).toTensor();
        float prob = output.item<float>();

        std::cout << "********************Test " << lineNum << "  → Probability: " << prob << "  → Predicted: " << (prob >= 0.5 ? 1 : 0) << endl;
        lineNum++;
    }
    infile.close();
